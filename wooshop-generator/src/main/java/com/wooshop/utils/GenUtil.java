...
import static com.wooshop.utils.FileUtil.SYS_TEM_DIR;
...
public class GenUtil {
    ...
    public static List<Map<String, Object>> preview(List<ColumnInfo> columns, GenConfig genConfig) {
        ...
        // 获取后端模版
        List<String> templates = getAdminTemplateNames();//ok
        TemplateEngine engine = TemplateUtil.createEngine(
                new TemplateConfig("template",
                        TemplateConfig.ResourceMode.CLASSPATH));
        for (String templateName : templates) {
            Map<String, Object> map = new HashMap<>(1);
            Template template = engine.getTemplate("generator/admin/" +
                    templateName + ".ftl");
    ...
    public static String download(List<ColumnInfo> columns,
        ...
        templates = getFrontTemplateNames();
        for (String templateName : templates) {
            Template template = engine.getTemplate("generator/front/" +
                    templateName + ".ftl");
    ...
    public static String downloadForElAdmin(List<ColumnInfo> columns, GenConfig genConfig) throws IOException {
        // 拼接的路径：/tmpeladmin-gen-temp/，这个路径在Linux下需要root用户才有权限创建,非root用户会权限错误而失败，更改为： /tmp/eladmin-gen-temp/
        // String tempPath =SYS_TEM_DIR + "eladmin-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
        String tempPath = SYS_TEM_DIR + "eladmin-gen-temp" + File.separator + genConfig.getTableName() + File.separator;
        Map<String, Object> genMap = getGenMap(columns, genConfig);
    ...
    /** ok **/
    public static void generatorCode(List<ColumnInfo> columnInfos,
                                     GenConfig genConfig) throws IOException {
        ...
        for (String templateName : templates) {
            Template template = engine.getTemplate("generator/front/" +
                    templateName + ".ftl");
            String filePath = getFrontFilePath(templateName,
                    genConfig.getApiPath(), genConfig.getPath(),
    ...
    public static void generatorCodeForEladmin(List<ColumnInfo> columnInfos,
                                               GenConfig genConfig) throws IOException {
        Map<String,Object> genMap = getGenMap(columnInfos, genConfig);
        TemplateEngine engine = TemplateUtil.createEngine(
                new TemplateConfig("template",
        ...
        List<String> templates = getAdminTemplateNamesForEladmin();
        for (String templateName : templates) {
            Template template = engine.getTemplate("generator/eladmin/"+
                    templateName+".ftl");
            String filePath = getAdminFilePathForEladmin(templateName,genConfig,
                    genMap.get("className").toString(),
                    System.getProperty("user.dir"));
    ...
    private static List<String> getAdminTemplateNamesForEladmin() {
        List<String> templateNames = new ArrayList<>();
        templateNames.add("EntityP");
        templateNames.add("DtoP");
        templateNames.add("MapperP");
    ...
    public static List<Map<String, Object>> previewForEladmin(List<ColumnInfo> columns, GenConfig genConfig) {
        Map<String,Object> genMap = getGenMap(columns, genConfig);
        List<Map<String,Object>> genList = new ArrayList<>();
        // 获取后端模版
        List<String> templates = getAdminTemplateNamesForEladmin();
     ...
     * 定义后端文件路径以及名称
     */
    private static String getAdminFilePathForEladmin(String templateName, GenConfig genConfig, String className, String rootPath) {
        ...
        if ("MapperXml".equals(templateName)) {
            return resourcePath + File.separator + "mapper" + File.separator + className + "Mapper.xml";
        }

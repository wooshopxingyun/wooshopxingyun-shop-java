  ...
  <#if betweens??>
    ...
    <#if columns??>
      ...
      <el-table ref="table" :default-sort = "{prop: 'index', order: 'descending'}" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <#if column.dictName??>
        <el-table-column prop="${column.changeColumnName}" label="<#if column.remark != ''>${column.remark}<#else>${column.changeColumnName}</#if>" :show-overflow-tooltip="true">
          <template slot-scope="scope">
...
const defaultCrud = CRUD({ title: '${apiAlias}', url: 'api/${changeClassName}', sort: '', crudMethod: { ...crud${className} }})
